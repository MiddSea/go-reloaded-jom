...
func PunctuationFormatDetailed(line string) string {
    ...
    consecutivePunctuation := regexp.MustCompile(`\s*([.,:;!?]{2,})\s*`)
   ...
   singlePunctuation := regexp.MustCompile(`\s*([.,!?])\s*([.,!?'])`)
...
//	aBfvowel_H := regexp.MustCompile(`([ \t]'?[aA])([ \t]'?[aeiouAEIOUhh])`)
	aBfvowel_H := regexp.MustCompile(`(^|\s'?[aA])(\s'?[aeiouAEIOUhH])`)
